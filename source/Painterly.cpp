	...
	for (int i = 0; i < layer_list.size(); i++)
	{
		...
		for (int x = margin_x; x < width - margin_x; x += (int)layer_list[i].grid_size)
		{
			for (int y = margin_y; y < height - margin_y; y += (int)layer_list[i].grid_size)
			{
				...
				int temp_area_length = 2 * (layer_list[i].grid_size / (double)2) + 1; // 무조건 홀수, 기본값대로라면 5

				temp_area.assign(temp_area_length, std::vector<int>(temp_area_length, 0));
				int RGB_diff = 0;

				if (temp_area_length <= 1) // 그런데 무조건 1이겠지. @+1이니까.
				{
				...
				{
					int temp_divide_two = (temp_area_length - 1) / 2; // 거의 무조건 짝수, 그냥 이렇게 해도 될듯.
					...
					area_error += (RGB_diff / (pow(temp_area_length, 2))); // 단순히 grid_size^를 빼면 안될것같음
				}
