...
{
	cv::Mat saving_saliency;

	int width = saliency_output.cols;
	int height = saliency_output.rows;

	saving_saliency.create(height, width, CV_8UC1);
	...
	for (int x = 0; x < width; x++)
	{
		...
		{
			float imsi = saliency_output.at<float>(y, x);
			if (max < saliency_output.at<float>(y, x)) max = saliency_output.at<float>(y, x);
		}
	}
	...
	for (int x = 0; x < width; x++)
	{
		for (int y = 0; y < height; y++)
		{
			//saving_saliency.at<uchar>(y, x) = (255.f / max) * saliency_output.at<float>(y, x);
			saving_saliency.at<uchar>(y, x) = (255.f / max) * saliency_output.at<float>(y, x);

		}
	}

	cv::imwrite("saliency_image.png", saving_saliency);
...
//에어브러시 O
cv::Mat stroke::paint_airbrush(float T, const cv::Mat& saliency_output, cv::Mat& canvas, const cv::Mat& reference, const std::vector<std::vector<int>> brush_vec, std::vector<layer>& layer_list, const std::vector<std::vector<float>>& image_etf_dx, const std::vector<std::vector<float>>& image_etf_dy)
{
		...
		for (int x = (margin_x + temp_divide_two); x < (width - margin_x - temp_divide_two); x += (int)layer_list[i].grid_size)
		{
			for (int y = (margin_y + temp_divide_two); y < (height - margin_y - temp_divide_two); y += (int)layer_list[i].grid_size)
			{
				...
				if (layer_list[i].grid_size <= 1) // if가 성립되면 무조건 그리드 사이즈가 1.
				{
					...
					average_brightness = saliency_output.at<float>(y, x); // average_brightness는 saliency에 저장된 floating point값이 저장되는 변수
